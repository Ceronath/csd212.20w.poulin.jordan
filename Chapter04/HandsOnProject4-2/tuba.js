...
function verifyAcres() {
   ...
   try {
      if (!(acresBox.value > 0)) {
         throw "Please enter acres greater than 0";
      }
   }
   catch (message) {
      acreVer = false;
      ...
      acresComplete = acreVer;
      messageElement.innerHTML = messText;
...
function verifyMonths() {
   ...
   try {
      if (monthsBox.value < 1 || monthsBox.value > 12) {
         throw "Please enter months between 1 and 12";
      }
   }
   catch (message) {
      monthsVer = false;
